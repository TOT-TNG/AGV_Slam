(function () {
  ...
  function boot(root) {
    ...
    const BASE_X = 80;
    const BASE_Y = 40;
    const SLOT_H = 130;   // khoảng cách cố định giữa các node
    const COLOR = "#3b82f6";
    ...
    function summarizeConfig(actionType, config) {
      const cfg = config || {};
      if (actionType === "MOVE_TO_POSE") return "Pose: " + (cfg.name || "-");
      if (actionType === "PICKUP") return "Lift";
      if (actionType === "DROP") return "Putdown";
    ...
    function WfArrowEdge(props) {
      ...
      const strokeWidth = (style && style.strokeWidth) || 2.5;
      const x = (sourceX + targetX) / 2;
      const arrowLen = 12;
      const arrowWid = 10;
      const startY = sourceY + 4;
    ...
    function TaskGroupNode({ id, data }) {
      ...
      return React.createElement(
        ...
        React.createElement(
          "button",
          { className: "wf-del", onClick: onDelete, title: "Remove" },
          "×"
        ),
